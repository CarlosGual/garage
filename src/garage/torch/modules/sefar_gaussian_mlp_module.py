...
from garage.torch.modules.sefar_mlp_module import SefarMLPModule


class SefarGaussianMLPModule(nn.Module):
    ...
    def __init__(self,
        ...
        self._init_std1 = torch.Tensor([init_std]).log()
        self._init_std2 = torch.Tensor([init_std]).log()
        log_std1 = torch.Tensor([init_std] * output_dim).log()
        log_std2 = torch.Tensor([init_std] * output_dim).log()
        if self._learn_std:
            self._log_std1 = torch.nn.Parameter(log_std1)
            self._log_std2 = torch.nn.Parameter(log_std2)
        else:
            self._log_std1 = log_std1
            self.register_buffer('log_std1', self._log_std1)
            self._log_std2 = log_std2
            self.register_buffer('log_std2', self._log_std2)
        ...
        self._mean_module = SefarMLPModule(
    ...
    def to(self, *args, **kwargs):
        ...
        if not isinstance(self._log_std, torch.nn.Parameter):
            self._log_std1 = buffers['log_std1']
            self._log_std2 = buffers['log_std2']
    ...
    def _get_mean_and_log_std(self, x):
        ...
        """
        means = self._mean_module(x)

        return means, [self._log_std1, self._log_std2]
    ...
    def forward(self, *inputs):
        ...
        """
        means, log_std_uncentereds = self._get_mean_and_log_std(*inputs)

        if self._min_std_param or self._max_std_param:
            log_std_uncentered1 = log_std_uncentereds[0].clamp(
            ...
            log_std_uncentered2 = log_std_uncentereds[1].clamp(
                min=(None if self._min_std_param is None else
                     self._min_std_param.item()),
                max=(None if self._max_std_param is None else
                     self._max_std_param.item()))
        else:
            log_std_uncentered1 = log_std_uncentereds[0]
            log_std_uncentered2 = log_std_uncentereds[1]

        if self._std_parameterization == 'exp':
            std1 = log_std_uncentered1.exp()
            std2 = log_std_uncentered2.exp()
        else:
            std1 = log_std_uncentered1.exp().exp().add(1.).log()
            std2 = log_std_uncentered2.exp().exp().add(1.).log()
        dist1 = self._norm_dist_class(means[0], std1)
        dist2 = self._norm_dist_class(means[1], std1)
        ...
        # are not accessable.
        if not isinstance(dist1, TanhNormal):
            # Makes it so that a sample from the distribution is treated as a
            # single sample and not dist.batch_shape samples.
            dist1 = Independent(dist1, 1)
        if not isinstance(dist2, TanhNormal):
            # Makes it so that a sample from the distribution is treated as a
            # single sample and not dist.batch_shape samples.
